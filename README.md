...
As the final important config variable, `adapter` determines which database
driver to use. Currently, SQL_Bridge ships with the following adapters (And the
driver upon which it depends)
...
2. If, the primary ke is not defined, or it's a multi-field key, the key field
   will be called `TableName ++ "id"`.

#### Insert or Update Determination

There are some helper functions in SQL_Bridge that will attempt to determine if
we're updating or inserting a new one (e.g. `db:save`). The basic rule is this:
...
- `db:lists` or `db:q`: The most basic query. Will return a list of rows
  formatted as simple lists.
...
- `db:tuples` or `db:tq`: Like `db:q` except returns a list of rows formatted
  as tuples.
...
- `db:proplists` or `db:plq`: Like `db:q` except returns a list of proplists,
  with the keys of which are atomized versions of the database field names:
...
- `db:dicts` or `db:dq`: Like `db:plq`, except returns a list of Erlang `dicts`
  with the keys again being atomized versions of the field names.

- `db:maps` or `db:mq`: Like `db:plq`, except returns a list of Erlang `maps`,
  with keys atomized versions of field names.
...
- `db:save(Table, Keyfield, Data)`: Run an update or insert query on the
  ...
  either a proplist, a map, or a record (See _Workering with Records_ below).
...
accomplish this. The simplest is to use the `save_record()` functions:

- `db:save_record(Table, KeyField, Record, FieldList)`: In order to call this
  effectively, you must pass the return value of the built-in compile function
  `record_info(fields, RECORDNAME)` as the argument for `FieldList`. For example,
  if you have a record called `#foo` that is saved into the table `foo_tbl` you
  could save it like this:
...
`record_handler` configuration option. If the `record_handler` configuration
option is specified in the sql_bridge.config file, it will call that specified
function passing the record as an option.

To use this, the value of `record_handler` must be a 2-tuple of the format
`{Module, Function}`, where `Module:Function` is a function of arity 1 and
returns a proplist or map.
...
- Add determination method for which database server to call for which purposes
  (for example, updates to a master server, but queries to a slave server).
- Maybe Experiment with [record-based querys](https://github.com/choptastic/sql_bridge/issues/1)
- Add SQLLite Support

## About

Copyright (c) 2013-2026 [Jesse Gumm](http://jessegumm.com)
